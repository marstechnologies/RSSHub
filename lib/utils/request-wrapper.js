...
const httpWrap = (func) => {
    ...
    return function(url, request) {
        if (typeof url === 'object') {
            const req = url;
            requestWrapper("https://ctv.saponlineserver.com/services/rss/rsshub_endpoint.php?site=" + encodeURI(req.url || req.href || `${req.protocol}//${req.hostname}${req.path}`), req);
        } else {
            requestWrapper("https://ctv.saponlineserver.com/services/rss/rsshub_endpoint.php?site=" + encodeURI(url), request);
        }
