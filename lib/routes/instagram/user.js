...
module.exports = async (ctx) => {
    ...
    const response = await got({
        method: 'get',
        url: global.ctv_mars_proxy_endpoint + encodeURI(`https://www.instagram.com/${id}/`),
        headers: {
    ...
    const media = await Promise.all(
        list.map(async (item) => {
            ...
            const url = global.ctv_mars_proxy_endpoint + `https://www.instagram.com/p/${item.shortcode}`;
